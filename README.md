...
> ⚠️ **重要**：请将应用安装到 `/Applications` 文件夹后使用，而不是直接从 build 目录运行。
> 
> 每次从 build 目录运行会生成新的签名，导致系统要求重新授权辅助功能权限。
> 
> 安装方法：
> ```bash
...
> ```
> 
> 然后从启动台或 Spotlight 启动 VoiceInput。

> 如果文字无法插入，请检查「系统设置」→「隐私与安全性」→「辅助功能」中是否已勾选 VoiceInput。
...
| 图标位置 | 来源 | 说明 |
|----------|------|------|
| 左侧第一个 | **macOS 系统** | 麦克风使用指示器，任何应用使用麦克风时自动显示 |
| 右侧第二个 | **VoiceInput** | 应用的菜单栏图标，点击控制录音 |

**macOS 麦克风指示器功能：**
- 显示当前正在使用麦克风的应用
- 点击可选择麦克风模式：
  - **标准模式** - 原始音频输入
  - **语音突显** - AI 降噪，过滤背景噪音，提升语音清晰度

> 💡 这是 macOS 14+ 的安全特性，用于保护用户隐私。停止录音后系统图标会自动消失。
